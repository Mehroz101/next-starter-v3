...
const Button = ({ loading, label = "Button" }: { loading: boolean, label: string }) => {
  return (
    <button
      type="submit"
      className={`w-full cursor-pointer bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-6 rounded-lg transition-colors duration-300 shadow-lg hover:shadow-xl"
              >  <span className='flex justify-center items-center gap-2 ${loading && "opacity-50 cursor-not-allowed"}`}
      disabled={loading}
    >
